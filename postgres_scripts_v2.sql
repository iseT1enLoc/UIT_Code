...
-- PART 5: CURSOR
-- PART 6: ROLE,GRANT,LOGIN
-- PART 7: VIEW
-- PART 8: PASSWORD ENCRYPTIONS
	-- password encryption using crypt module
	-- hash_function(plain_text,gen_salt(5_optional_encrypt_algorithm))
------------------------------
...
--1-Insert reservation given time,customer_id,...
...
RETURNS TABLE(customer_id INT,customer_name VARCHAR(30),booking_times BIGINT)
AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION get_customer_booking_history_by_customer_id(p_customer_id INT)
RETURNS TABLE (created_date DATE, time_begin TIME, time_end TIME, field_id INT, price MONEY)
AS $$
BEGIN
    IF EXISTS (SELECT * FROM reservation res JOIN conformation con ON res.reservation_id = con.reservation_id WHERE res.customer_id = p_customer_id) THEN
        RETURN QUERY
        SELECT res.created_date, res.time_begin, res.time_end, res.field_id, res.renting_price
        FROM conformation con
        JOIN reservation res ON con.reservation_id = res.reservation_id
        WHERE res.customer_id = p_customer_id
		ORDER BY RES.match_date DESC;
    ELSE
        PERFORM display_message('There is no customer with the given id');
        RETURN;
    END IF;
END;
$$ LANGUAGE plpgsql;
drop function get_customer_booking_history_by_customer_id(p_customer_id INT);
select * from get_customer_booking_history_by_customer_id(3);

--PART 5. CURSOR
--1.cursor trong procedure để cập nhật loại khách hàng dựa trên số lần đặt sân.
ALTER TABLE CUSTOMER ADD customer_type VARCHAR(20) DEFAULT 'bình thường';
...
DECLARE 
	total_times INT;
	cus_id BIGINT;
	customer_cursor cursor for select cus.customer_id from CUSTOMER cus;
begin
	-- Open the cursor
    ...
    LOOP
		...
		WHERE RES.reservation_id in(select con.reservation_id from conformation con) AND RES.customer_id = cus_id;
		
        UPDATE customer
		SET customer_type = CASE
								WHEN total_times<normal THEN 'Bình thường'
                				WHEN total_times >= normal AND total_times<medium THEN 'Hạng trung'
								ELSE'Thân thiết'
             				END
...
LANGUAGE plpgsql ;
call customer_type_updating(1,2);
drop procedure customer_type_updating;
select * from customer;


--PART 6: ROLE,GRANT,LOGIN
--a.CREATE ROLE
--create role for owner of stadium
CREATE ROLE "owner_stadium_role";
...
--b.CREATE USER AND GRANT PERMISSION TO USER
CREATE USER employee_user WITH PASSWORD 'StrongP&ssword';
GRANT "employee_role" TO "employee_user";

CREATE USER customer_user WITH PASSWORD 'Str*ngPassword';
GRANT "customer_role" TO "customer_user";

CREATE USER manager_user WITH PASSWORD 'StrongP@ssw0rd';
GRANT "owner_stadium_role" TO "manager_user";

--C.GRANT ON TABLE, PROCEDURE,FUNCTION
--C.1."owner_stadium_role"
--tables
GRANT ALL ON CONFORMATION TO "owner_stadium_role" WITH GRANT OPTION;

--limit the selected columns on customer columns when interact as manager on customer
--can not view password
GRANT SELECT(customer_id, customer_name,gmail,phone_number,date_of_birth,customer_type) on CUSTOMER TO "owner_stadium_role";

GRANT ALL ON DRINK TO "owner_stadium_role" WITH GRANT OPTION;

GRANT SELECT,UPDATE,INSERT ON DRINK_ITEM TO "owner_stadium_role" WITH GRANT OPTION;

GRANT ALL ON DRINK_ORDER TO "owner_stadium_role" WITH GRANT OPTION;

GRANT SELECT(employee_id,employee_name,phone_number,date_of_birth,gmail),INSERT,UPDATE,DELETE ON EMPLOYEE TO "owner_stadium_role" WITH GRANT OPTION;

GRANT ALL ON FIELD TO "owner_stadium_role" WITH GRANT OPTION;

GRANT ALL ON FIELD_TYPE TO "owner_stadium_role" WITH GRANT OPTION; 

GRANT ALL ON PAYMENT TO "owner_stadium_role" WITH GRANT OPTION;

GRANT SELECT,INSERT,UPDATE ON RESERVATION TO "owner_stadium_role" WITH GRANT OPTION;

--stored procedure
GRANT EXECUTE ON PROCEDURE add_a_conformation TO "owner_stadium_role" WITH GRANT OPTION;
GRANT EXECUTE ON PROCEDURE add_a_reservation TO "owner_stadium_role" WITH GRANT OPTION;
GRANT EXECUTE ON PROCEDURE customer_type_updating TO "owner_stadium_role" WITH GRANT OPTION;
GRANT EXECUTE ON PROCEDURE display_message TO "owner_stadium_role" WITH GRANT OPTION;
--function
GRANT EXECUTE ON FUNCTION get_already_booked_field_by_date TO "owner_stadium_role" WITH GRANT OPTION;
GRANT EXECUTE ON FUNCTION get_customer_booking_history_by_customer_id TO "owner_stadium_role" WITH GRANT OPTION;
GRANT EXECUTE ON FUNCTION get_customer_by_id TO "owner_stadium_role" WITH GRANT OPTION;
GRANT EXECUTE ON FUNCTION get_customer_list TO "owner_stadium_role" WITH GRANT OPTION;
GRANT EXECUTE ON FUNCTION get_top_k_bestseller_in_month_x TO "owner_stadium_role" WITH GRANT OPTION;
GRANT EXECUTE ON FUNCTION get_top_x_customer_year TO "owner_stadium_role" WITH GRANT OPTION;

--C.2."employee_role"
--TABLE
GRANT ALL ON CONFORMATION TO "employee_role" WITH GRANT OPTION;

--limited can not see customer password;
GRANT SELECT(customer_id, customer_name,gmail,phone_number,date_of_birth,customer_type) ON CUSTOMER TO "employee_role" WITH GRANT OPTION;

GRANT ALL ON DRINK TO "employee_role" WITH GRANT OPTION;

GRANT SELECT, INSERT,UPDATE ON DRINK_ITEM TO "employee_role" WITH GRANT OPTION;

GRANT SELECT,UPDATE,INSERT,DELETE ON DRINK TO "employee_role" WITH GRANT OPTION;

GRANT ALL ON DRINK_ORDER TO "employee_role";
--can not see password
GRANT SELECT(employee_id,employee_name,phone_number,date_of_birth,gmail),INSERT,UPDATE,DELETE ON EMPLOYEE TO "employee_role";
GRANT SELECT,UPDATE,INSERT ON PAYMENT TO "employee_role";
GRANT ALL ON RESERVATION TO "employee_role";

--procedure
GRANT EXECUTE ON PROCEDURE add_a_conformation TO "employee_role" WITH GRANT OPTION;
GRANT EXECUTE ON PROCEDURE add_a_reservation TO "employee_role" WITH GRANT OPTION;
GRANT EXECUTE ON PROCEDURE customer_type_updating TO "employee_role" WITH GRANT OPTION;
GRANT EXECUTE ON PROCEDURE display_message TO "employee_role" WITH GRANT OPTION;

--function
GRANT EXECUTE ON FUNCTION get_already_booked_field_by_date TO "employee_role" WITH GRANT OPTION;
GRANT EXECUTE ON FUNCTION get_customer_booking_history_by_customer_id TO "employee_role" WITH GRANT OPTION;
GRANT EXECUTE ON FUNCTION get_customer_by_id TO "employee_role" WITH GRANT OPTION;
GRANT EXECUTE ON FUNCTION get_customer_list TO "employee_role" WITH GRANT OPTION;
GRANT EXECUTE ON FUNCTION get_top_k_bestseller_in_month_x TO "employee_role" WITH GRANT OPTION;
GRANT EXECUTE ON FUNCTION get_top_x_customer_year TO "employee_role" WITH GRANT OPTION;

--C.2."customer_role"
--tables
GRANT ALL ON CONFORMATION TO "customer_role" WITH GRANT OPTION;
GRANT SELECT(customer_id, customer_name,gmail,phone_number,date_of_birth,customer_type),INSERT,UPDATE,DELETE ON CUSTOMER TO "customer_role" WITH GRANT OPTION;
GRANT SELECT ON PAYMENT TO "customer_role" WITH GRANT OPTION;
GRANT ALL ON CONFORMATION TO "customer_role" WITH GRANT OPTION;

--procedures
GRANT EXECUTE ON PROCEDURE add_a_conformation TO "customer_role" WITH GRANT OPTION;
GRANT EXECUTE ON PROCEDURE add_a_reservation TO "customer_role" WITH GRANT OPTION;

--function
GRANT EXECUTE ON FUNCTION get_already_booked_field_by_date TO "customer_role" WITH GRANT OPTION;
GRANT EXECUTE ON FUNCTION get_customer_booking_history_by_customer_id TO "customer_role" WITH GRANT OPTION;

-- PART 7: VIEW
--7.1 VIEW TO SEE TOP FIVE CUSTOMER WHO PAY THE MOST IN THIS MONTH
CREATE OR REPLACE VIEW TOP_PAYMENT_CUSTOMER_OF_THIS_MONTH AS
SELECT CUS.customer_id,CUS.customer_name, sum(PAY.total_price) as total_bill_pay
FROM PAYMENT PAY, CONFORMATION CON, RESERVATION RES, CUSTOMER CUS
WHERE PAY.conformation_id = CON.conformation_id 
		AND CON.reservation_id = RES.reservation_id 
		AND RES.customer_id = CUS.customer_id AND extract(month from CON.created_date) = extract(month from current_date)
GROUP BY CUS.customer_id
ORDER BY SUM(PAY.total_price) DESC
LIMIT 5
--7.2  View to see historic payment method behavior of customer
CREATE OR REPLACE VIEW VIEW_HISTORIC_PAYMENT_METHOD_BEHAVIOR AS
SELECT CASE 
			WHEN PAY.payment_method = 1 THEN 'Tiền mặt'
			WHEN PAY.payment_method = 2 THEN 'Ngân hàng'
			ELSE 'momo'
		END AS Payment_method,
		count(payment_id) as total
FROM PAYMENT PAY
GROUP BY PAY.payment_method
--7.3 View for hiding imported price of drink
CREATE OR REPLACE VIEW DRINK_HIDE_IMPORT_PRICE_VIEW AS
SELECT drink_id,drink_name,selling_price,number_in_stock FROM DRINK WITH CHECK OPTION;
--7.4 View for showing already booked field today
CREATE OR REPLACE VIEW VIEW_ALREADY_BOOKED_FIELD_TODAY AS
SELECT * FROM get_already_booked_field_by_date(current_date);

--7.5 View for hiding sensitive information in database
--customer
CREATE OR REPLACE VIEW VIEWABLE_CUS_INFOR AS 
SELECT customer_id, customer_name, gmail, phone_number, date_of_birth,customer_type
FROM CUSTOMER WITH CHECK OPTION;
--employee
CREATE OR REPLACE VIEW VIEWABLE_EM_INFOR AS
SELECT employee_id,employee_name,phone_number,date_of_birth,gmail,pass_word from EMPLOYEE WITH CHECK OPTION;
--7.6 View to see the most drink consumtion this month
CREATE OR REPLACE VIEW VIEW_OF_BEST_SELLER_DRINK_THIS_MONTH AS
select dr.drink_id,dr.drink_name,sum(item.quantity) as total_quantity
from drink_item item 
join drink dr on item.drink_id = dr.drink_id
where item.drink_order_id in (select item1.drink_order_id from drink_order item1 where extract(month from item1.created_date) = extract(month from current_date))
group by dr.drink_id,dr.drink_name

--PART 8: PASSWORD ENCRYPTION
CREATE EXTENSION pgcrypto;
UPDATE CUSTOMER
SET pass_word = crypt(customer_name,gen_salt('md5')) 
alter table customer alter column pass_word type varchar(50);
--1.CUSTOMER
